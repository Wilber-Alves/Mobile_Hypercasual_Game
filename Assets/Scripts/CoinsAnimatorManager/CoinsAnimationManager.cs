...
{
    // this change allowns to add the list at the begining of the game,
    // and then start the animation when we want,
    // for example, when the player collect the first coin,
    // or when the player reach the end of the level, etc.
    
    public List<ItemCollectableBase> PFB_Coin = new List<ItemCollectableBase>(); 
    ...
    public void RegisterCoin(ItemCollectableBase i)
    {
    ...
    IEnumerator ScalePiecesByTime()
    {
        ...
        for (int i = 0; i < PFB_Coin.Count; i++)
        {
            ...
            {   
                ...
                PFB_Coin[i].transform.DOScale(targetScale, scaleDuration).SetEase(scaleEase);
